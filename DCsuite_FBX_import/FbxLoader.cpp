...
FBXLoader::FBXLoader(const char * pFileName)
	: mFileName(pFileName),SdkManager(NULL), Scene(NULL),Importer(NULL), CurrentAnimLayer(NULL),SelectedNode(NULL),
	  mPoseIndex(-1),mSupportVBO(true) ,  mAnimTotalFrame(0)
	...
	for (int lIndex=0; lIndex<lNodeCount; lIndex++)
	{
		...
		if (lNode->GetGeometry()) 
		{
			...
			for (i=0; i<lVertexCacheDeformerCount; ++i )
			{
				...
				fbxsdk::FbxVertexCacheDeformer* lDeformer = static_cast<fbxsdk::FbxVertexCacheDeformer*>(lNode->GetGeometry()->GetDeformer(i, FbxDeformer::eVertexCache));
...
bool FBXLoader::LoadTextureFromFile(const FbxString & pFilePath, unsigned int & pTextureObject)
{
	...
	wstring w_file_name = StringLibrary::string2wstring(pFilePath.Buffer());
	dcsbodyspace::TextureMap_DCBody::create_gl_texture(w_file_name);
	return false;
}
...
void FBXLoader::LoadCacheRecursive(FbxScene * pScene, FbxAnimLayer * pAnimLayer, const char * pFbxFileName, bool pSupportVBO)
{
	...
	for (int lTextureIndex = 0; lTextureIndex < lTextureCount; ++lTextureIndex)
	{
		...
		if (lFileTexture && !lFileTexture->GetUserDataPtr())
		{
			...
			const fbxsdk::FbxString lFileName = lFileTexture->GetFileName();
			...
			{
				int texture_index = dcsbodyspace::TextureMap_DCBody::create_gl_texture(FbxStringtoDCWC(lFileName) ); //<<---texture
				const fbxsdk::FbxString lAbsFbxFileName = FbxPathUtils::Resolve(pFbxFileName);
				const fbxsdk::FbxString lAbsFolderName = FbxPathUtils::GetFolderName(lAbsFbxFileName);
				wstring Texture_path;
				if (texture_index == -1)
				{
					// load texture from relative file name (relative to FBX file)
					const fbxsdk::FbxString lResolvedFileName = FbxPathUtils::Bind(lAbsFolderName, lFileTexture->GetRelativeFileName());
					texture_index = dcsbodyspace::TextureMap_DCBody::create_gl_texture(FbxStringtoDCWC(lResolvedFileName) );
					Texture_path = FbxStringtoDCWC(lResolvedFileName);
				}
				if (texture_index == -1)
				{
					// load texture from file name only (relative to FBX file)
					const fbxsdk::FbxString lTextureFileName = FbxPathUtils::GetFileName(lFileName);
					const fbxsdk::FbxString lResolvedFileName = FbxPathUtils::Bind(lAbsFolderName, lTextureFileName);
					texture_index = dcsbodyspace::TextureMap_DCBody::create_gl_texture(FbxStringtoDCWC(lResolvedFileName) );
					Texture_path = FbxStringtoDCWC(lResolvedFileName);
				}
				if (texture_index == -1)
				{
					// load texture from .fbm folder (original to FBX file path)
					const fbxsdk::FbxString lTextureFileName = FbxPathUtils::GetFileName(lFileName);
					fbxsdk::FbxString lFbmFodler = FbxString(pFbxFileName);
					lFbmFodler.FindAndReplace(".fbx" , ".fbm");
					const fbxsdk::FbxString lResolvedFileName = FbxPathUtils::Bind(lFbmFodler, lTextureFileName);
					texture_index = dcsbodyspace::TextureMap_DCBody::create_gl_texture(FbxStringtoDCWC(lResolvedFileName) );
					Texture_path = FbxStringtoDCWC(lResolvedFileName);
				}
				if (texture_index != -1)
				{
					lTextureObject = GLuint(texture_index);
					lStatus = true;
					texturePathList.push_back(Texture_path);
				}
				else
				{
					lStatus = false;
...
bool FBXLoader::LoadFile()
{
	...
	if (Importer->Import(Scene) == true)
	{
		...
		bool trianglulate_bool = lGeomConverter.Triangulate(Scene,true,true);//node //<<------------------------------------------timing more..
		
		// Split mMeshes per material, so that we only have one material per mesh (for VBO support)
		
		// Bake the scene for one frame
		LoadCacheRecursive(Scene, CurrentAnimLayer, mFileName, mSupportVBO);
...
bool FBXLoader::SetCurrentAnimStack(int pIndex)
{
	...
	mAnimTotalFrame = int(( mStop.Get() - mStart.Get() )/mFrameTime.Get());
...
bool FBXLoader::Draw(int frame ,const int iOption)
{
	FbxPose * lPose = NULL;
	if (mPoseIndex != -1)
	{
	...
	//set current time
	FbxTime tempTime = mStart + mFrameTime * frame;

	mCurrentTime = ( tempTime > mStop ) ? mStop : tempTime;

	if (SelectedNode)
	{
		DrawNodeRecursive(SelectedNode, mCurrentTime, 
			CurrentAnimLayer, lDummyGlobalPosition, lPose , iOption);
	}
	// Otherwise, draw the whole scene.
	else
	{
		DrawNodeRecursive(Scene->GetRootNode(), mCurrentTime,
			CurrentAnimLayer, lDummyGlobalPosition, lPose , iOption);
	}
								  ...
								  FbxAMatrix& pParentGlobalPosition, FbxPose* pPose ,const int iOption)
{
	...
	if (pNode->GetNodeAttribute())
	{
		...
		DrawNode(pNode, pTime, pAnimLayer, pParentGlobalPosition,
			lGlobalOffPosition, pPose , iOption);
	...
	for (int lChildIndex = 0; lChildIndex < lChildCount; ++lChildIndex)
	{
		DrawNodeRecursive(pNode->GetChild(lChildIndex), pTime, pAnimLayer, lGlobalPosition, pPose , iOption);
	}
						 ...
						 FbxAMatrix& pGlobalPosition,FbxPose* pPose ,const int iOption)
{
	...
	if (lNodeAttribute)
	{
		if (lNodeAttribute->GetAttributeType() == FbxNodeAttribute::eSkeleton)
		{
			...
			//Do not draw skeleton
		}
		// NURBS and patch have been converted into triangluation mMeshes.
		else if (lNodeAttribute->GetAttributeType() == FbxNodeAttribute::eMesh)
		{
			//DrawMesh(pNode, pTime, pAnimLayer, pGlobalPosition, pPose);	
			DrawMesh(pNode, pTime, pAnimLayer, pGlobalPosition, pPose , iOption);
		}
...
void FBXLoader::DrawMesh(FbxNode* pNode, FbxTime& pTime, FbxAnimLayer* pAnimLayer,
						 FbxAMatrix& pGlobalPosition, FbxPose* pPose ,const int iOption)
{
	...
	FbxVector4* lVertexArray = NULL;	// for current frame vertex array
	FbxVector4* lVertexArray_nf = NULL;	// for next frame vertex array
	// deform vertex & save in lVertexArray  
	// DC mesh collider �� ������ ����  

	UpdateMeshVert(pNode,pTime,pAnimLayer,
		pGlobalPosition,pPose,lVertexArray , iOption);
	



	if (iOption == NEXT_FRAME)
		return;
	...
	if (lMeshCache)
	{
		...
		for (int lIndex = 0; lIndex < lSubMeshCount; ++lIndex)
		{
			...
			if (lMaterial)
			{
				...
				if (lMaterialCache && lMaterialCache->HasTexture())	//texture ������ ������ default material��
				{
					lMaterialCache->SetCurrentMaterial();
				}
				else
				{
					lMaterialCache->SetDefaultMaterial();
				}
			}
			else
			{
	...
	delete [] lVertexArray;
	delete [] lVertexArray_nf;

}
void FBXLoader::UpdateMeshVert(FbxNode* pNode, FbxTime& pTime, FbxAnimLayer* pAnimLayer,
								FbxAMatrix& pGlobalPosition, FbxPose* pPose ,
								FbxVector4* lVertexArray , const int iOption )
{
	...
	if (lHasDeformation)
	{
		...
		if (lMeshCache)
			lMeshCache->UpdateVertexPosition(lMesh, pGlobalPosition,  lVertexArray , iOption);
	}
...
void FBXLoader::ReadVertexCacheData(FbxMesh* pMesh,  FbxTime& pTime, FbxVector4* pVertexArray)
{
	...
	fbxsdk::FbxVertexCacheDeformer* lDeformer     = static_cast<fbxsdk::FbxVertexCacheDeformer*>(pMesh->GetDeformer(0, FbxDeformer::eVertexCache));
	FbxCache*               lCache        = lDeformer->GetCache();
	int                      lChannelIndex = -1;
	...
	{
		if ((lChannelIndex = lCache->GetChannelIndex(lDeformer->GetCache()->GetName())) > -1)
		{
...
bool VBOMesh::Initialize(FbxMesh *pMesh)
{
	if (!pMesh->GetNode())
		return false;
	...
	if (!mHasNormal)
	{
		std::cout<<"WARNING : Has no normal , create noraml automatically , BAD Quality!!!"<<std::endl;
		if(pMesh->GenerateNormals(false , true , false) && pMesh->GetElementNormalCount() > 0)	//�߰��� false �̸� ���� ������ render
			mHasNormal = true;
	}
	...
	//for dc collider // ����ȭ
	float * lColliderVertices = new float [ pMesh->GetControlPointsCount() * VERTEX_STRIDE];
	unsigned int * lColliderIndices = new unsigned int[lPolygonCount * TRIANGLE_VERTEX_COUNT];

	for (int lIndex = 0; lIndex < pMesh->GetControlPointsCount(); ++lIndex)
	{
		// Save the vertex position.
		lCurrentVertex = lControlPoints[lIndex];
		lColliderVertices[lIndex * VERTEX_STRIDE] = static_cast<float>(lCurrentVertex[0]);
		lColliderVertices[lIndex * VERTEX_STRIDE + 1] = static_cast<float>(lCurrentVertex[1]);
		lColliderVertices[lIndex * VERTEX_STRIDE + 2] = static_cast<float>(lCurrentVertex[2]);
		lColliderVertices[lIndex * VERTEX_STRIDE + 3] = 1;
	}
	...
	for (int lPolygonIndex = 0; lPolygonIndex < lPolygonCount; ++lPolygonIndex)
	{
		for (int lVerticeIndex = 0; lVerticeIndex < TRIANGLE_VERTEX_COUNT; ++lVerticeIndex)
		{
			...
			lColliderIndices[ lPolygonIndex*3 + lVerticeIndex] = static_cast<unsigned int>(lControlPointIndex);
		}
	}

	// For dc collider
	mNumPolyIndex = lPolygonCount ;
	mNumVert =  pMesh->GetControlPointsCount();
	mColliderDCMesh = new MeshStruct();
	mColliderDCMesh_nf = new MeshStruct();
	SetColliderIndex( lColliderIndices  );
	SetColliderPosNorm( lColliderVertices ,CURRENT_FRAME );
	SetColliderPosNorm( lColliderVertices ,NEXT_FRAME );
	delete [] lColliderIndices;
	delete [] lColliderVertices;
...
void VBOMesh::UpdateVertexPosition(const FbxMesh * pMesh,FbxAMatrix& pGlobalPosition, const FbxVector4 * pVertices ,const int iOption ) const
{
	//for DC collider mesh
	float * colliderVert = new float[ pMesh->GetControlPointsCount() * VERTEX_STRIDE];
	for (int lIndex = 0; lIndex < pMesh->GetControlPointsCount(); ++lIndex)
	{
		FbxVector4 TransformedVector = pGlobalPosition.MultT(pVertices[lIndex]) ;
		colliderVert[lIndex * VERTEX_STRIDE] = static_cast<float>(TransformedVector[0]);
		colliderVert[lIndex * VERTEX_STRIDE + 1] = static_cast<float>(TransformedVector[1]);
		colliderVert[lIndex * VERTEX_STRIDE + 2] = static_cast<float>(TransformedVector[2]);
		colliderVert[lIndex * VERTEX_STRIDE + 3] = 1;
	}
	SetColliderPosNorm(colliderVert , iOption);
	delete [] colliderVert;
	//end DC collider mesh 
...
void VBOMesh::SetColliderIndex(unsigned int * mIndices ) const
{
	
	mColliderDCMesh->Init_All();
	mColliderDCMesh ->numVertices =	(int)mNumVert;
	mColliderDCMesh ->numNormals =	(int)mNumVert;
	mColliderDCMesh ->numFaces =	mNumPolyIndex;
	
	ArrayNi v2f_temp;
	v2f_temp.Resize(mColliderDCMesh->numFaces * 3);
	for (int i = 0; i < int(mColliderDCMesh->numFaces); i++)
	{
		v2f_temp.GetPtr()[i*3] =	static_cast<int>(mIndices[i*3]);
		v2f_temp.GetPtr()[i*3+1] =	static_cast<int>(mIndices[i*3+1]);
		v2f_temp.GetPtr()[i*3+2] =	static_cast<int>(mIndices[i*3+2]);
	}
	mColliderDCMesh->v2f.Resize_memcpy(v2f_temp);	
	//mColliderDCMesh->SetConnectivityData();
	
	mColliderDCMesh_nf->Set_All(mColliderDCMesh);

	v2f_temp.Release();
}
void VBOMesh::SetColliderPosNorm(float * mVertices , const int iOption) const
{

	ArrayN<Vector3r ,0 > posVert ;
	posVert.Resize(mNumVert); 
	for (int i = 0; i < int(mNumVert); i++)
	{
		Vector3<double> temp_v( static_cast<float>(mVertices[i*VERTEX_STRIDE])
			, static_cast<float>(mVertices[i*VERTEX_STRIDE+1])
			, static_cast<float>(mVertices[i*VERTEX_STRIDE+2]) );

		posVert.GetPtr()[i] = temp_v;
	}
	if (iOption == CURRENT_FRAME)
	{
		mColliderDCMesh->posVert.Resize_memcpy(posVert);
		mColliderDCMesh->posVert_ori.Resize_memcpy(posVert);
		mColliderDCMesh->GetNormals();
	}
	else if (iOption == NEXT_FRAME)
	{
		mColliderDCMesh_nf->posVert.Resize_memcpy(posVert);
		mColliderDCMesh_nf->posVert_ori.Resize_memcpy(posVert);
		mColliderDCMesh_nf->GetNormals();
	}
...
{
	glDisable(GL_COLOR_MATERIAL);	//color material ���� ����ε� material �Դ´�
	const GLfloat AMBIENT_COLOR[] = {0.2,0.2,0.2,1.0};
	const GLfloat DIFFUSE_COLOR[] = {0.7,0.7,0.7,1.0};
	const GLfloat SPECULAR_COLOR[] = {1.0,1.0,1.0,1.0};
	const GLfloat BLACK_COLOR[] = {0 , 0 , 0 , 0};
...
DCBODYDLL const wchar_t * FbxStringtoDCWC(fbxsdk::FbxString inStr)
{
	//FbxString �� DC �� WC �� �ٲٴ� ���
	__wchar_t * temp_wc;
	fbxsdk::FbxUTF8ToWC(inStr.Buffer(),temp_wc);
	const wchar_t * result = reinterpret_cast<const wchar_t*>(temp_wc);
	return result;
}
